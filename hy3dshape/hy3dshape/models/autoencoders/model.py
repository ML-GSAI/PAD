...
class VectsetVAE(nn.Module):
    ...
    def latents2sdf(self, latents: torch.FloatTensor, **kwargs):
        with synchronize_timer('Volume decoding'):
            grid_logits = self.volume_decoder(latents, self.geo_decoder, **kwargs)
        return grid_logits
    
    def query_sdf(self, latents, points: torch.FloatTensor):
        # latents: 1, 4096, 1024
        # query points: 1, 8000, 3
        sdf = self.geo_decoder(queries=points, latents=latents)
        sdf = rearrange(sdf, 'b n 1 -> b n')
        return sdf
    
    def latents2grid(self, latents, **kwargs):
        with synchronize_timer('Volume decoding'):
            grid_logits = self.volume_decoder(latents, self.geo_decoder, **kwargs)
        return grid_logits
...
class ShapeVAE(VectsetVAE):
    ...
    def encode_none(self, surface, sample_posterior=True, ret_moments=False):
        pc, feats = surface[:, :, :3], None
    ...
    def encode(self, surface, sample_posterior=True, ret_moments=False):
    ...
    def sample_posterior(self, moments, sample_posterior=True):
        posterior = DiagonalGaussianDistribution(moments, feat_dim=-1)
        if sample_posterior:
            latents = posterior.sample()
        else:
