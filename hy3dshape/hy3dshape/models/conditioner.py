...
class ImageEncoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if config is None:
            print("image enc version:", version)
...
class SingleImageEncoder(nn.Module):
    ...
    def forward(self, image, mask=None, **kwargs):
        ...
        # print("[SingleImageEncoder] image range:", image.min(), image.max())
        outputs = {
            'main': self.main_image_encoder(image, mask=mask, **kwargs),
        }
...
class ImageDepthEncoder(nn.Module):
    ...
    def forward(self, image, mask=None, **kwargs):
        ...
        if depth is not None:
            ...
            outputs["depth"] = self.main_image_encoder(depth, mask=mask, **kwargs)
        else:
            print("Warning: depth is None in ImageDepthEncoder forward!")
        if self.disable_drop:
            return outputs
        else:
            random_p = torch.rand(len(image), device='cuda')
    ...
    def unconditional_embedding(self, batch_size, **kwargs):
        outputs = {
            'main': self.main_image_encoder.unconditional_embedding(batch_size, **kwargs),
            'depth': self.main_image_encoder.unconditional_embedding(batch_size, **kwargs),
        }
        return outputs
