...
class Hunyuan3DDiTPipeline:
    ...
    def __init__(
        ...
        **kwargs
        ...
        print("creating pipeline with dtype:", dtype)
        self.vae = vae
        self.model = model
    ...
    def __call__(
        self,
        image: Union[str, List[str], Image.Image, torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if kwargs.get('latents', None) is not None:
            cond["latents"] = kwargs.pop('latents')
...
class Hunyuan3DDiTFlowMatchingPipeline(Hunyuan3DDiTPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print('before prepare: image', type(image), 'mask', type(mask))
        ...
        if 'pts' in kwargs:
            cond["pts"] = kwargs.pop('pts')
            if do_classifier_free_guidance:
                cond["pts"] = torch.cat([cond["pts"]] * 2, dim=0)
...
class Hunyuan3DDiTDepthFlowMatchingPipeline(Hunyuan3DDiTPipeline):

    @classmethod
    @synchronize_timer('Hunyuan3DDiTPipeline Model Loading')
    def from_single_file_init(
        cls,
        ckpt_path,
        config_path,
        device='cuda',
        ...
        **kwargs,
        ...
        if use_safetensors:
            ...
            ckpt = torch.load(ckpt_path, map_location='cpu', weights_only=True)
        # load model
        config["model"]["params"]["depth_cond"] = True
        model = instantiate_from_config(config['model'])
        model = model.load_partial_ckpt(model, ckpt['model'])
        vae = instantiate_from_config(config['vae'])
        vae.load_state_dict(ckpt['vae'], strict=False)
        conditioner = instantiate_from_config(config['conditioner'])
        if 'conditioner' in ckpt:
    ...
    def prepare_ccm(self, depth):
        if isinstance(depth, torch.Tensor):
            return depth

        if isinstance(depth, str) and not os.path.exists(depth):
            raise FileNotFoundError(f"Couldn't find depth at path {depth}")
        ...
        for d in depth:
            if isinstance(d, torch.Tensor):
                ...
                if isinstance(d, str):
                    d = Image.open(d).convert('L')
                elif isinstance(d, dict) and 'depth_map' in d:
                    d = d['depth_map']
                depth_map = self.image_processor.depth_processor(d)
    ...
    def __call__(
        self,
        image: Union[str, List[str], Image.Image, dict, List[dict], torch.Tensor] = None,
        depth: Union[str, List[str], Image.Image, dict, List[dict], torch.Tensor] = None,
        num_inference_steps: int = 50,
        timesteps: List[int] = None,
        sigmas: List[float] = None,
        eta: float = 0.0,
        ...
        **kwargs,
        ...
        cond_inputs = self.prepare_image(image, mask)
        image = cond_inputs.pop('image')
        cond_inputs["depth"] = self.prepare_ccm(depth)
